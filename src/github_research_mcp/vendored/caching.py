...
CachableValueTypes = ToolResult | list[Tool] | list[Resource] | list[Prompt] | list[ReadResourceContents] | GetPromptResult
...
class ToolResultCacheEntry(BaseCacheEntry):
    ...
    def validate_value(cls, value: dict[str, Any] | ToolResult) -> ToolResult:
        ...
        content_block_type_adapter: TypeAdapter[list[mcp.types.ContentBlock]] = get_cached_typeadapter(list[mcp.types.ContentBlock])
...
class CachablePrompt(Prompt):
    async def render(
        ...
        raise NotImplementedError("Prompt.render() is not implemented on cached prompts")
...
class ListPromptsCacheEntry(BaseCacheEntry):
    ...
    @field_validator("value", mode="before")
    @classmethod
    def validate_value(cls, value: Sequence[dict[str, Any]] | Sequence[Prompt]) -> Sequence[CachablePrompt]:
        results = []
...
class CachableResource(Resource):
    async def read(self) -> str | bytes:
        ...
        raise NotImplementedError("Resource.read() is not implemented on cached resources")
...
class ListResourcesCacheEntry(BaseCacheEntry):
    ...
    @field_validator("value", mode="before")
    @classmethod
    def validate_value(cls, value: Sequence[dict[str, Any]] | Sequence[Resource]) -> Sequence[Resource]:
        results = []
        for item in value:
...
class ReadResourceCacheEntry(BaseCacheEntry):
    ...
    @field_validator("value", mode="before")
    @classmethod
    def validate_value(cls, value: Sequence[dict[str, Any]] | Sequence[ReadResourceContents]) -> Sequence[ReadResourceContents]:
        ...
        for item in value:
            ...
            resource_contents.append(ReadResourceContents(content=content, mime_type=mime_type))

        return resource_contents

    @field_serializer("value")
    def serialize_value(self, value: Sequence[ReadResourceContents]) -> list[dict[str, Any]]:
        return [
            {
...
class DiskCache(CacheProtocol):
    ...
    def __init__(
        ...
        *,
        ...
        self._cache = disk_cache or DiskCacheClient(directory=path, size_limit=size_limit)
    ...
    async def set_entry(
        ...
        collection_key: str = self.make_collection_key(collection=cache_entry.collection, key=cache_entry.key)
...
class InMemoryCache(CacheProtocol):
    ...
    def __init__(self, max_entries: int = DEFAULT_MEMORY_CACHE_MAX_ENTRIES):
        ...
        self._cache: TLRUCache[Any, CacheEntryTypes] = MemoryCacheClient[Any, CacheEntryTypes](
            maxsize=max_entries,
...
class ResponseCachingMiddleware(Middleware):
    ...
    def __init__(
        ...
        self.method_settings: MethodSettings = method_settings or DEFAULT_METHOD_SETTINGS
    ...
    async def on_read_resource(
        ...
        call_next: CallNext[mcp.types.ReadResourceRequestParams, list[ReadResourceContents]],
    ...
    async def on_get_prompt(
        ...
        call_next: CallNext[mcp.types.GetPromptRequestParams, mcp.types.GetPromptResult],
    ...
    async def _get_cache(
        ...
        if cached_entry := await self._backend.get_entry(collection=collection, key=key):
    ...
    def _matches_tool_cache_settings(self, context: MiddlewareContext[mcp.types.CallToolRequestParams]) -> bool:
    ...
    def _get_cache_settings(
        ...
        method_settings_key = MCP_METHOD_TO_METHOD_SETTINGS_KEY.get(context.method, None)

        if method_settings_key is None or method_settings_key not in self.method_settings:
            return None
    ...
    def _get_cache_ttl(self, context: MiddlewareContext[Any]) -> int:
        ...
        settings: SharedMethodSettings | None = self._get_cache_settings(context=context)
...
def get_size_of_tool_result(value: ToolResult) -> int:
    ...
    structured_content_size = len(json.dumps(value.structured_content, sort_keys=True, separators=(",", ":")).encode("utf-8"))
